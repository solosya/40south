...
$("document").ready(function () {
  ...
  Acme.adScroll = function () {
    ...
    if (
      $(".j-article-container").length > 0 &&
      $(".j-article-sidebar").length > 0 &&
      $(window).width() > 991
    ) {
      ...
      if (scrollMetric[1] === "up" && !isScrolledPast(articleTop - 135)) {
        articleAd
          .removeClass("advertisment__side-fix--fixed")
          .removeClass("advertisment__side-fix--bottom")
          .addClass("advertisment__side-fix--top");
      } else if (
        scrollMetric[1] === "up" &&
        !isScrolledPast(theHeight - screenDiff + articleTop)
      ) {
        articleAd
          .removeClass("advertisment__side-fix--bottom")
          .addClass("advertisment__side-fix--fixed");
      } else if (
        scrollMetric[1] === "down" &&
        isScrolledPast(theHeight - screenDiff + articleTop)
      ) {
        articleAd
          .removeClass("advertisment__side-fix--fixed")
          .removeClass("advertisment__side-fix--top")
          .addClass("advertisment__side-fix--bottom");
      } else if (
        scrollMetric[1] === "down" &&
        isScrolledPast(articleTop - 135)
      ) {
        articleAd
          .removeClass("advertisment__side-fix--top")
          .addClass("advertisment__side-fix--fixed");
  ...
  $(".js-menu").on("click", function (event) {
    ...
    if (iOS()) {
      ...
      var iOSelements = $(
        $("body")
          .children()
          .not("script, .responsive-standalone, .responsive-standalone-overlay")
      );
  ...
  $(".list-arrow-container-sub").on("click", function (e) {
    ...
    $(".custom-menu-child").each(function () {
      ...
      elem.removeClass("active");
      elem
        .find(".custom-menu-child-sub")
        .removeClass("custom-menu-child-sub--active");
    });
    if (!isActive) {
      $parent.addClass("active");
      $(this)
        .siblings(".custom-menu-child-sub")
        .addClass("custom-menu-child-sub--active");
    }
  });
